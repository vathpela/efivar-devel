...
static void
action_add(list_t *list, action_type_t action_type,
...
main(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv, sopts, lopts, &i)) != -1) {
		...
		case 'c':
			...
			action_add(&actions, mode, &owner, X509_CERT, data, datasz);
			break;
		case 'd':
			dump = true;
			break;
		...
		if (rc < 0) {
			...
			if (siglistsz > 4
				...
				rc = efi_secdb_parse(&siglist[4], siglistsz-4,
						     &secdb);
			if (rc < 0)
				secdb_err(1, "could not parse input file \"%s\"", infile);
		}
		xfree(siglist);
		list_del(&entry->list);
		free(entry);
