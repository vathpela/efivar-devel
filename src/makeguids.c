...
cmpnamep(const void *p1, const void *p2)
{
	...
	printf("%s: gn1->name:%s gn2->name:%s\n", __func__, gn1->name, gn2->name);
...
""");
	...
	qsort(outbuf, line, sizeof (struct guidname), cmpguidp);
	rc = write(guidout, outbuf, sizeof (struct guidname) * line);
	if (rc < 0)
		err(1, "could not write guids.bin");

	qsort(outbuf, line, sizeof (struct guidname), cmpnamep);
	rc = write(nameout, outbuf, sizeof (struct guidname) * line);
	if (rc < 0)
