...
secdb_buffer(char *val, size_t valsz, size_t offset)
{
	char *nbuf;
...
secdb_dump_value(char *val, size_t size, size_t offset, char *fmt, ...)
{
	char posbuf[9];
	char hexbuf[49];
	char textbuf[19];
	posbuf[sizeof(hexbuf)-1] = '\0';
	...
	while (size - printed) {
		...
		printf("\n");
		//printf("size %zd - printed %zd = %zd\n", size, printed, size-printed);
	}
...
static inline size_t
secdb_dump_esl(efi_secdb_t *secdb, int esl, size_t offset)
{
	...
	offset = secdb_dump_value((char *)NULL, secdb->hdrsz, offset,
				  "esl[%d].signature_header (end:0x%08x)",
...
static inline size_t
secdb_dump_esd(secdb_entry_t *entry, int esl, int esd,
